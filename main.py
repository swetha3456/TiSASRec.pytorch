...
# --- NEW: LOAD CATEGORY MAPPINGS ---
print("Loading category mappings for hard negative sampling...")
with open("data/category_mapping.json", "r") as f:
    raw_mapping = json.load(f)["product_to_category"]
    # Cast keys to int to match the training IDs
    item_to_cat = {int(k): v for k, v in raw_mapping.items()}

cat_to_items_set = defaultdict(set)
for item_id, cat_id in item_to_cat.items():
    cat_to_items_set[cat_id].add(item_id)
# -----------------------------------
...
# --- UPDATED: Pass mappings to Sampler ---
sampler = WarpSampler(
    user_train, usernum, itemnum, relation_matrix, 
    batch_size=args.batch_size, maxlen=args.maxlen, n_workers=3,
    cat_to_items_set=cat_to_items_set, item_to_cat=item_to_cat # Add these
)
